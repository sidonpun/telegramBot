...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        translations["start"]["en"], reply_markup=ask_language()
    )
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                translations["menu_website"][lang], url="https://desync.pro/"
            )
        ],
        [
            InlineKeyboardButton(
                translations["menu_game"][lang],
                callback_data="menu_choose_game",
            )
        ],
        [
            InlineKeyboardButton(
                translations["menu_loader"][lang],
                callback_data="download_loader",
            )
        ],
        [
            InlineKeyboardButton(
                translations["menu_status"][lang],
                url="https://desync.pro/statuses",
            )
        ],
        [
            InlineKeyboardButton(
                translations["menu_faq"][lang], callback_data="faq"
            )
        ],
        [
            InlineKeyboardButton(
                translations["menu_support"][lang], callback_data="support"
            )
        ],
        [
            InlineKeyboardButton(
...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "menu_choose_game":
        ...
        await send_loader_info(update, context)
...
async def game_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    game = games[gid]
    keyboard = [
        [InlineKeyboardButton(f"{d} days", callback_data=f"sub_{d}")]
        for d in game["durations"]
    ]
    keyboard.append(
        [
            InlineKeyboardButton(
                translations["menu_instruction"][lang], callback_data="guide"
            )
        ]
    )
    keyboard.append(
        [
            InlineKeyboardButton(
                translations["back"][lang], callback_data="back_to_main"
            )
        ]
    )
    await query.edit_message_text(
        translations["choose_subscription"][lang],
        reply_markup=InlineKeyboardMarkup(keyboard),
    )
...
async def subscription_selected(
    ...
    text = translations["subscription_result"][lang].format(
        title=game["title"],
        days=days,
        desc=game["description"][lang],
        link=game["links"][days],
    )
    await query.edit_message_text(
        text,
        parse_mode="Markdown",
...
async def guide_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        text, parse_mode="Markdown", reply_markup=back_to_main_button(lang)
    )
...
async def send_faq_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if doc_url:
        await query.edit_message_text(
            f"📄 {doc_url}", reply_markup=back_to_main_button(lang)
        )
    else:
        await query.edit_message_text(
            "❌ Вопрос не найден.", reply_markup=back_to_main_button(lang)
        )
...
async def show_games(message, user_id):
    ...
    keyboard = [
        [InlineKeyboardButton(g["title"], callback_data=f"choose_{gid}")]
...
async def support_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    support_text = "💬 *Support contacts:*\n" + "\n".join(
        f"• {name}" for name in SUPPORT_CONTACTS
    )
    await query.edit_message_text(
        support_text,
        parse_mode="Markdown",
...
def main():
    ...
    app.add_handler(
        CallbackQueryHandler(
            partial(
                handle_language_selection, show_main_menu_fn=show_main_menu
            ),
            pattern="^lang_",
        )
    )
    ...
    app.add_handler(
        CallbackQueryHandler(subscription_selected, pattern="^sub_")
    )
    ...
    app.add_handler(
        CallbackQueryHandler(send_loader_info, pattern="^download_loader$")
    )
    app.add_handler(
